...
2. **[Command entrypoints](layers/cmd.md)** — `cmd/api`, `cmd/grpc`, `cmd/worker`, `cmd/migrate`
3. **[Application layer](layers/app.md)** — bootstrap, router, servers, middleware, lifecycle
4. **[Configuration](layers/config.md)** — Viper config structs and `config.yaml.example`
5. **Domains**
   - [Sample domain](layers/domain-sample.md) — CRUD, cache-aside, outbox events
   - [Healthcheck domain](layers/domain-healthcheck.md) — dependency ping aggregation
6. **[Infrastructure](layers/infrastructure.md)** — databases, Kafka, outbox relay, observability
...
10. **[Root and tooling](root-and-tooling.md)** — Makefile, Dockerfile, `go.mod`

## Architecture decision records

- [ADR 001: Transactional Outbox for Kafka Publish](adr/001-transactional-outbox.md) — why outbox relay + consumer, Mongo direct-write trade-offs
...
| [root-and-tooling.md](root-and-tooling.md) | Makefile, Docker, scripts, module |
